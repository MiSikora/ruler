...
class DistanceFormatterTest {
  ...
  @Test fun `formatting uses provided separator`() {
    ...
    val formattedDistance = distance.format(context, unitSeparator = "-")
  ...
  @Test fun `coerced formatting uses kilometers for large values with SI units`() {
    val length = Length.ofGigameters(1)
    val distance = length.toDistance(Gigameter)

    val formattedDistance = distance.coercedFormat(context)

    formattedDistance shouldBe "1000000.00km"
  }

  @Test fun `coerced formatting uses meters for small values with SI units`() {
  ...
  @Test fun `formatting uses provided coercer`() {
    val length = Length.ofMillimeters(987)
    val distance = length.toDistance(Millimeter)
    ...
    class NanoCoercer<T>
      ...
      override fun coerce(distance: Distance<T>)= when (distance.unit) {
        is SiDistanceUnit -> (distance as Distance<SiDistanceUnit>).coerceUnitAtMostTo(Nanometer)
        else -> distance
      }
    }

    val formattedDistance = distance.coercedFormat(context, coercer = NanoCoercer())
